...
def userview_complaint(request):
    lid = request.POST['lid']
    l=[]
    data=Complaint.objects.filter(USERS__AURH_USER=lid)
    for i in data:
        l.append({
            'id':i.id,
...
def usersend_complaint(request):
    id=request.POST['lid']
    ...
    data.replay='pending'
    data.status='pending'
    data.USERS=Users.objects.get(AURH_USER=id)
    data.save()
    return JsonResponse({'status': 'ok'})

def userview_forestdivision(request):
...
def notifi(request):
    ...
    d=Detect_Alert.objects.filter(id__gt=nid, date= datetime.datetime.now())
    if d.exists():
        d=d.first()
        return JsonResponse(
            {
...
@csrf_exempt
def user_viewfarmer(request):
    l=[]
    lid=request.POST['lid']
    data=Users.objects.all().exclude(AURH_USER_id=lid)
    for i in data:
        l.append({
            'id':i.id,
            'name':i.name,
...
@csrf_exempt
def user_viewproducts(request):
    ownerid=request.POST['ownerid']

    l = []
    data = Product.objects.filter(USERS_id=ownerid)
    for i in data:
        l.append({
            'id': i.id,
            'Product_name': i.name,
            'Product_photo': i.image,
            'description': i.description,
            'Price': i.price,
...
def user_addquantity(request):
    ...
    a.USERS=Users.objects.get(AURH_USER_id=lid)
    a.save()
    return JsonResponse({'status': 'ok'})
...
def user_viewcart(request):
    ...
    l = []
    data = Cart.objects.filter(USERS__AURH_USER_id=lid)
    for i in data:
        l.append({
            'id': i.id,
            'Date': i.date,
            ...
            'Product_photo': i.PRODUCT.image,
            'Product_id': i.PRODUCT.id,
            'Farmer_id': i.PRODUCT.USERS.id,
...
def user_place_order(request):
    if request.method == "POST":
            ...
            items = request.POST.get("items")   # JSON string

            print("ITEMS RECEIVED:", items)

            item_list = json.loads(items)

            # take farmer id from first product
            farmer_id = item_list[0]['farmer_id']

            # ---- SAVE ORDER MAIN ----
            om = Ordermain()
            om.BUYING_USERS = Users.objects.get(AURH_USER=lid)
            # om.PRODUCT_OWNER_id = Users.objects.get(id=farmer_id)
            om.PRODUCT_OWNER = Users.objects.get(id=farmer_id)
            om.total_amount = total_amount
            om.Date = date.today()
            om.Status = "paid"
            om.save()
            ...
            for i in item_list:
                ...
                os = Ordersub()
                os.ORDERMAIN_id = om.id           # use correct FK name
                os.PRODUCT = product
                os.Quantity = i['Quantity']
                os.Price = i['product_price']
                os.Subtotal = i['subtotal']
                os.save()
...
@csrf_exempt
def user_clear_cart(request):
    if request.method == "POST":
        lid = request.POST.get("lid")

        user = Users.   objects.get(AURH_USER=lid)

        Cart.objects.filter(USERS=user).delete()
...
#
#         if not lid or not total_amount or not items:
#             print("missing fields")
#             return JsonResponse({"status": "missing fields"})
#
#         item_list = json.loads(items)
#
#         if len(item_list) == 0:
#             print("cart empty")
#             return JsonResponse({"status": "cart empty"})
#
#         # Get buyer
#         buyer = Users.objects.get(AURH_USER=lid)
#
#         # Get farmer from first product
#         farmer_id = item_list[0]['farmer_id']
#         farmer = Users.objects.get(id=farmer_id)
#
...
def user_vieworder(request):
    ...
    l = []
    data = Ordermain.objects.filter(USERS__AUTH_USER_id=lid)
    for i in data:
        l.append({
            'id': i.id,
...
@csrf_exempt
def user_vieworderitem(request):
    ...
    for i in data:
        l.append({
            ...
            'Product_price': i.PRODUCT.Price,
            'Product_photo': i.PRODUCT.Product_photo,
            'pid': i.PRODUCT.id,
...
def viweanimaldtect(request):
    dd=Detect_Alert.objects.filter(date= datetime.datetime.now())
    l=[]
    for d in dd:
        # d=d.first()
        l.append({
